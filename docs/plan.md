...
flowchart TD
  ...
  G --> H[Bağımsız doğrulama ve imzalar; satıcı varlık alabilir]
...
Demo sırasında alım doğrulaması imzalı test verisidir; gerçek satıcı veya tapu entegrasyonu değildir. Kontrat fon transferini doğrular, fiziksel mülkiyet devrini kendi başına bilemez. Zaman aşımı zincirde otomatik işlem başlatmaz; süre dolunca herkes iptal/iade çağrısını yapabilir.
